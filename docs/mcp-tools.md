...
```json
{ "bookId": "shared-reading-guide", "chunkId": "ch00" }
```
...
```json
{ "bookId": "shared-reading-guide", "query": "values", "limit": 10 }
```
...
```json
{ "bookId": "shared-reading-guide", "confirm": true }
```
...
```json
{
  ...
  "quote": "this line matters",
  "quoteOffset": 42,
  "note": "I want AI assistant to comment on this.",
  "author": "user",
  "status": "open"
}
```
...
```json
{ "bookId": "shared-reading-guide", "chunkId": "ch00", "kind": "resonance", "status": "published" }
```

All fields are optional except `bookId` in most practical use.

By default this tool hides human notes with `status: "open"`, `"private"`, or `"draft"` and hides replies under those private notes. This lets a reader save notes and leave without exposing them to AI assistant. A trusted local UI can pass `includePrivate: true`; AI assistant-facing MCP clients should not.
...
Finds all user annotations with `status: "open"`, `"private"`, or `"draft"`, returns them as one batch for AI assistant, and rewrites them to `status: "submitted"` so the same notes are not sent again.

This is the tool a companion app or AI assistant-facing integration should call when the user taps “Send notes to AI assistant”. In the built-in browser reader, this action publishes the private notes so AI assistant can retrieve them through MCP and returns the context package to the HTTP caller; it is not a push notification into your AI client by itself.
...
- `chunk-once-per-session`: default; send each chunk once per AI assistant session
...
```json
{ "bookId": "shared-reading-guide", "limit": 10 }
```
...
```json
{
  ...
  "note": "AI assistant's reply under this user note.",
  "kind": "reply",
  "status": "published"
}
```

AI assistant-facing MCP replies are published immediately. Human replies typed in the built-in web reader are created through the HTTP API as `status: "open"` and stay staged until the reader clicks `Send to AI assistant`.

Creates a AI assistant annotation with `parentId` pointing to the original note. If `bookId`, `chunkId`, or `quote` are omitted, they are inherited from the parent annotation.
...
`finish.celebration` is intentionally small and variable. AI assistant can use it as a closing ritual, or a frontend can ignore it and keep only the stable counts.
...
Returns only unread card prompts, such as “收获了一枚回声书签”, with the `cardId` needed to open or dismiss them. This is the reader-facing flow AI assistant should use instead of reading raw card data.
...
Returns the selected card as image content so AI assistant can view it directly. If Playwright Chromium is installed, the card is rendered as a polished PNG from the same HTML/CSS card template used by the reader; otherwise the server falls back to an SVG image. The text part is only a short caption.
