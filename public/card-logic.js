...
export function isAssistantAuthor(author) {
...
export function findSharedMoments(annotations = []) {
  ...
  const assistants = roots.filter((annotation) => isAssistantAuthor(annotation.author));
  const moments = [];
  for (const userNote of human) {
    for (const assistantNote of assistants) {
      if (!quotesOverlap(userNote.quote, assistantNote.quote)) continue;
      const quote = userNote.quote.length <= assistantNote.quote.length ? userNote.quote : assistantNote.quote;
      moments.push({
        id: `${userNote.id}:${assistantNote.id}`,
        quote,
        userNote,
        assistantNote,
      });
...
export function sharedNoteIdSet(annotations = []) {
  ...
  for (const moment of findSharedMoments(annotations)) {
    ...
    ids.add(moment.assistantNote.id);
  }
  return ids;
}
...
export function buildCardCandidates({ book = {}, chunk = {}, annotations = [], finish = null } = {}) {
  ...
  for (const [index, moment] of shared.entries()) {
    candidates.push({
      ...
      artSeed: hashText(`${moment.userNote.id}:${moment.assistantNote.id}:${moment.quote}`),
      ...
      leftLabel: "Assistant",
      leftText: compactText(moment.assistantNote.note, 130),
      rightLabel: "Reader",
  ...
  if (resonant && !lowSignal) {
    candidates.push({
      ...
      leftLabel: isAssistantAuthor(resonant.author) ? "Assistant" : "Reader",
