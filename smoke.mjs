...
const read = await request("tools/call", {
  ...
  arguments: { bookId: "shared-reading-guide", chunkId: "ch00" },
});
const search = await request("tools/call", {
  name: "reading_search_chunks",
  arguments: { bookId: "shared-reading-guide", query: "共读" },
});
const continueGuide = await request("tools/call", {
  name: "reading_continue",
  arguments: { bookId: "shared-reading-guide" },
});
const importText = [
  "Chapter Alpha",
  "",
...
const firstSubmit = await request("tools/call", {
  ...
  arguments: { bookId: "shared-reading-guide", sessionId: "session-a" },
});
...
const newSessionSubmit = await request("tools/call", {
  name: "reading_submit_user_notes",
  arguments: { bookId: "shared-reading-guide", sessionId: "session-b" },
});
...
const reply = await request("tools/call", {
  ...
  arguments: { parentId: "ann_guide_user_001", note: "The assistant can answer in the margin." },
});
const replyId = contentJson(reply).id;
const nestedReply = await request("tools/call", {
  name: "reading_reply_to_annotation",
  arguments: { parentId: replyId, note: "The assistant can also answer a reply in the same thread." },
});
const replies = await request("tools/call", {
  name: "reading_list_annotations",
  arguments: { parentId: "ann_guide_user_001" },
});
...
const badMarkRead = await request("tools/call", {
  name: "reading_mark_read",
  arguments: { bookId: "shared-reading-guide", chunkId: "missing-chunk" },
});
...
const httpChunk = await fetchJson("/api/books/shared-reading-guide/chunks/ch00");
const repeatedQuote = "共读";
...
const httpReplyToAssistant = await fetchJson("/api/replies", {
  method: "POST",
  body: {
    parentId: "ann_guide_assistant_001",
    note: "HTTP reader reply to the assistant should remain staged.",
    author: "user",
  },
});
...
const httpCards = await fetchJson("/api/cards?bookId=shared-reading-guide");
const httpCardInbox = await fetchJson("/api/card-inbox?bookId=shared-reading-guide");
const httpCardCollection = await fetchJson("/api/card-collection?bookId=shared-reading-guide&limit=1");
...
if (!list.result?.content?.[0]?.text.includes("shared-reading-guide")) {
  throw new Error("reading_list_books did not return shared-reading-guide");
}
if (contentJson(list).find((book) => book.bookId === "shared-reading-guide")?.chunksRead !== 1) {
  throw new Error("reading_list_books counted stale progress chunk ids");
}
if (!read.result?.content?.[0]?.text.includes("欢迎来到共读空间")) {
  throw new Error("reading_read_chunk did not return chunk text");
}
if (!search.result?.content?.[0]?.text.includes("共读")) {
  throw new Error("reading_search_chunks did not return a values snippet");
}
if (contentJson(continueGuide).chunk.id !== "ch01") {
...
if (!contentJson(visibleAfterSubmit).some((note) => note.id === "ann_guide_user_001")) {
  throw new Error("reading_list_annotations did not expose submitted human notes");
}
if (!contentJson(firstSubmit).context.chunks[0]?.text.includes("欢迎来到共读空间")) {
  throw new Error("first session submit did not include chunk text");
}
if (contentJson(mcpSpoofNote).author !== "assistant" || contentJson(mcpSpoofNote).status !== "published") {
...
if (!contentJson(newSessionSubmit).context.chunks[0]?.text.includes("欢迎来到共读空间")) {
...
if (!reply.result?.content?.[0]?.text.includes('"parentId": "ann_guide_user_001"')) {
  throw new Error("reading_reply_to_annotation did not attach to the parent annotation");
}
if (!replies.result?.content?.[0]?.text.includes("The assistant can answer in the margin")) {
...
if (!nestedReplies.result?.content?.[0]?.text.includes("The assistant can also answer a reply")) {
...
if (!httpBooks.some((book) => book.bookId === "shared-reading-guide")) {
  throw new Error("HTTP API did not list shared-reading-guide");
}
if (httpMcpInit.result?.serverInfo?.name !== "co-reading-mcp") {
  throw new Error("HTTP process did not keep MCP stdio active");
}
if (!httpChunk.text.includes("欢迎来到共读空间")) {
...
if (httpReplyToAssistant.parentId !== "ann_guide_assistant_001" || httpReplyToAssistant.status !== "open") {
  throw new Error("HTTP reader replies to the assistant annotations should stay staged until submit");
}
...
if (!sseApiBooks.some((book) => book.bookId === "shared-reading-guide")) {
