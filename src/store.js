...
const roomConfigDefaults = Object.freeze({
  configured: false,
  roomName: "共读空间",
  readerName: "我",
  partnerName: "共读伙伴",
...
function normalizeRoomConfig(input = {}, current = roomConfigDefaults) {
  return {
    configured: Boolean(input.configured ?? current.configured),
    roomName: cleanRoomText(input.roomName, current.roomName),
...
export async function getRoomConfig() {
  return normalizeRoomConfig(await readJson(roomConfigPath, roomConfigDefaults));
}
...
function chooseBookFinishMoment(roots, replies) {
  ...
  const threaded = roots
    .map((root) => {
      ...
      const hasAssistant = thread.some((item) => isAssistantAuthor(item.author));
      const noteSource =
        thread.filter((item) => isAssistantAuthor(item.author)).sort((a, b) => momentScore(b) - momentScore(a))[0]
        || thread.sort((a, b) => momentScore(b) - momentScore(a))[0];
      return { root, thread, noteSource, hasHuman, hasAssistant, score: momentScore(root, thread.slice(1)) };
    })
    .filter((item) => item.hasHuman && item.hasAssistant && String(item.root.quote || "").trim())
  ...
  const paired = Array.from(byChunk.values())
    .filter((items) => items.some((item) => isHumanAuthor(item.author)) && items.some((item) => isAssistantAuthor(item.author)))
    .flatMap((items) => items.filter((item) => isAssistantAuthor(item.author)).concat(items.filter((item) => isHumanAuthor(item.author))).slice(0, 1))
  ...
  const assistant = roots
    .filter((item) => isAssistantAuthor(item.author) && String(item.quote || "").trim())
    .sort((a, b) => momentScore(b) - momentScore(a));
  if (assistant[0]) return { root: assistant[0], noteSource: assistant[0], shared: false, reason: "assistant-margin" };
...
function isAssistantAuthor(author) {
  ...
  return !isHumanAuthor(value) && (!value || value === "assistant" || value === "claude" || value === "ember");
}
...
export async function annotatePassage(input) {
  return withWriteLock(async () => {
    ...
    const author = input.author || "assistant";
