...
export const tools = [
  ...
  },
  {
    ...
    description: "Write an assistant margin annotation anchored to a quote in a chunk. Human private notes should be created through the HTTP reader API, not this MCP tool.",
    inputSchema: {
      type: "object",
  ...
  },
  {
    ...
    description:
      "Submit open user notes for the assistant to review. By default, include each chunk's full text once per session and mark notes submitted so they are not sent again.",
    inputSchema: {
      type: "object",
      properties: {
        bookId: { type: "string" },
  ...
  },
  {
    ...
    description: "List human note submission batches that have been shared with the assistant.",
    inputSchema: {
      type: "object",
      properties: {
        bookId: { type: "string" },
  ...
  },
  {
    ...
    description: "Attach an assistant reply under an existing user or assistant annotation.",
    inputSchema: {
      type: "object",
  ...
  },
  {
    ...
    description: "Open one collected reading card and return it as an image for the assistant to view.",
    inputSchema: {
      type: "object",
      required: ["cardId"],
...
export async function callTool(name, args = {}) {
  switch (name) {
    ...
    case "reading_annotate_passage":
      return textContent(await annotatePassage({ ...args, author: "assistant", status: "published" }));
    ...
    case "reading_reply_to_annotation":
      return textContent(await replyToAnnotation({ ...args, author: "assistant", status: "published" }));
  ...
  if (message.method === "initialize") {
    return result(message.id, {
      ...
      instructions:
        ...
        `The assistant can import EPUB/TXT uploads, continue reading, read chunked books, search passages, track progress, leave margin annotations, ` +
